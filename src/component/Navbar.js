...
// its a raect router which allow user to implement dynamic routing and only update required setion not reload whole page 
// also declare in parent Class which is App.js
...
const Navbar = () => {
  return (
    <div className='nav'>
      {/* link tag is similar to anchor tag why? in anchor tag as clicked page is loaded and directed to desiered page but in link tag  routes are setted so that only required setion is rendered while other are no loaded */}
      <div className="logo">
        <a href='/'>
          <img id="logoImage" src={logo} alt="Not" />
        </a>
      </div>
        <a href="/miphones" className='navlinks'>Mi Phone</a>
